...
/**
 *   	\file       fournreadfile_list.php
 *		\ingroup    fournreadinvoice
 *		\brief      List page for fournreadfile
 */
...
require_once __DIR__.'/class/fournreadfile.class.php';
// for other modules
...
$diroutputmassaction = $conf->fournreadinvoice->dir_output.'/temp/massgeneration/'.$user->id;
...
if (!isModEnabled("fournreadinvoice")) {
	accessforbidden('Module fournreadinvoice not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (!GETPOST('confirmmassaction', 'alpha') && $massaction != 'presend' && $massaction != 'confirm_presend' && $massaction != 'analyze') {
	$massaction = '';
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Fournreadfile';
	$objectlabel = 'Fournreadfile';
	$uploaddir = $conf->fournreadinvoice->dir_output;
...
$title = $langs->trans("Fournreadfiles");
//$help_url = "EN:Module_Fournreadfile|FR:Module_Fournreadfile_FR|ES:Módulo_Fournreadfile";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/fournreadinvoice/fournreadfile_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-fournreadinvoice page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
if (GETPOSTINT('nomassaction') || in_array($massaction, array('presend', 'predelete', 'analyze'))) {
	$arrayofmassactions = array();
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/fournreadinvoice/fournreadfile_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendFournreadfileRef";
$modelmail = "fournreadfile";
$objecttmp = new Fournreadfile($db);
