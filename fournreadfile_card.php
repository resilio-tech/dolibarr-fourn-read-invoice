...
/**
 *    \file       fournreadfile_card.php
 *    \ingroup    fournreadinvoice
 *    \brief      Page to create/edit/view fournreadfile
 */
...
dol_include_once('/fournreadinvoice/lib/fournreadinvoice_fournreadfile.lib.php');
require_once DOL_DOCUMENT_ROOT .'/core/lib/files.lib.php';

// Load translation files required by the page
$langs->loadLangs(array("fournreadinvoice@fournreadinvoice", "other"));
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->hasRight('fournreadinvoice', 'fournreadfile', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('fournreadinvoice', 'fournreadfile', 'write'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('fournreadinvoice', 'fournreadfile', 'write'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->fournreadinvoice->multidir_output[isset($object->entity) ? $object->entity : 1].'/fournreadfile';
...
if (empty($reshook)) {
	$backurlforlist = dol_buildpath('/fournreadinvoice/fournreadfile_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/fournreadinvoice/fournreadfile_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
	...
	if ($action == 'add') {
		...
		if (empty($filenameTmp) || !move_uploaded_file($file['tmp_name'], $upload_file)) {
			...
			if (is_array($result)) {
				$error++;
				setEventMessages(null ,$result,'errors');
				$action = 'create';
			} else {
				setEventMessages($langs->trans("FileUploaded"), null, 'mesgs');
				header('Location: '.dol_buildpath('/fournreadinvoice/fournreadfile_list.php', 1));
			}
		}
	}
}
...
$title = $langs->trans("Fournreadfile")." - ".$langs->trans('Card');
//$title = $object->ref." - ".$langs->trans('Card');
if ($action == 'create') {
	$title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("Fournreadfile"));
}
$help_url = '';

llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-fournreadinvoice page-card');
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Fournreadfile"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	$head = fournreadfilePrepareHead($object);

	print dol_get_fiche_head($head, 'card', $langs->trans("Fournreadfile"), -1, $object->picto, 0, '', '', 0, '', 1);

	$formconfirm = '';

	// Confirmation to delete (using preloaded confirm popup)
	if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteFournreadfile'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/fournreadinvoice/fournreadfile_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
...
//		$linktoelem = $form->showLinkToObjectBlock($object, null, array('fournreadfile'));
...
//		$MAXEVENT = 10;
//
//		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/fournreadinvoice/fournreadfile_agenda.php', 1).'?id='.$object->id);
//
...
//
//	// Presend form
//	$modelmail = 'fournreadfile';
//	$defaulttopic = 'InformationMessage';
//	$diroutput = $conf->fournreadinvoice->dir_output;
//	$trackid = 'fournreadfile'.$object->id;
